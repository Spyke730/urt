...
use EstadatSAS\Forms\ChangePasswordForm;
use EstadatSAS\Forms\UsersForm;
use EstadatSAS\Models\Users;
use EstadatSAS\Models\PasswordChanges;
use EstadatSAS\Models\Adscribe;

/**
 * EstadatSAS\Controllers\UsersController
...
class UsersController extends ControllerBase
{
	...
	public function searchAction()
	{
		...
		if (count($users) == 0) {
			$this->flash->notice("En la búsqueda no se encontró ningún usuario");
	...
	public function editAction($id)
	{
		...
		if ($this->request->isPost()) {
			...
			if (!$user->save()) {
				...
				$this->flash->success("El usuario fue actualizado");
	...
	public function deleteAction($id)
	{
		$user = Users::findFirstById($id);
		if (!$user) {
			$this->flash->error("Usuario no econtrado");
			return $this->dispatcher->forward(array(
				'action' => 'index'
			));
		}
		...
		if (!$user->delete()) {
			...
			$this->flash->success("El usario fue eliminado");
		}
		...
		if ($this->request->isPost()) {
			...
			if (!$form->isValid($this->request->getPost())) {
				...
				if (!$passwordChange->save()) {
					...
					$this->flash->success('Su contraseña se cambio exitosamente');
