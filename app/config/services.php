...
use EstadatSAS\Auth\Auth;
use EstadatSAS\Acl\Acl;
use EstadatSAS\Mail\Mail;
...
$di->set('dispatcher', function () {
	...
	$dispatcher->setDefaultNamespace('EstadatSAS\Controllers');
	return $dispatcher;
});
...
$di->set('viewCache', function () use ($config){
	// Get the parameters
	$frontCache      = new PhCacheFront(array('lifetime' => 84600));
	if (function_exists('apc_store')) {
		$cache = new PhCacheBackApc($frontCache);
	} else {
		$backEndOptions = array('cacheDir' => $config->application->cacheDir);
